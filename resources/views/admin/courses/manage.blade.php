                    ...
                    <div x-transition.duration.500ms x-show="isExpanded" class="mt-2 rounded-md border p-2 transition-all">
                        <ul>
                            @forelse($question->answers as $index => $answer)
                                <li class="{{$answer->is_correct == 5  ? 'font-bold text-green-400' : ''}}" >{{chr(65 + $index)}}. {{$answer->answer}} <span class="bg-blue-200 rounded-md text-white px-2">Poin {{ $answer->is_correct}}</span></li>
                            @empty
                            @endforelse
                        </ul>
                    </div>
