<x-app-layout>
    <div class="flex justify-between items-center mb-6">
      ...
      <a class="py-2 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-md border border-transparent bg-blue-600 text-white hover:bg-blue-700 "
        href={{route("dashboard.packages.edit", $package)}}>Simpan</a>
    </div>
    {{-- <div class="">
        ...
        @forelse($courses as $course)
            <div class="border p-4 gap-2 rounded-lg flex flex-col bg-white">
                ...
                <div class="flex justify-between gap-2">
                    <form action="{{route('dashboard.add_course_to_package')}}" method="POST">
                        @csrf
                        <input name="package_id" class="hidden" type="text" value="{{$package->id}}">
                        <input name="course_id" class="hidden" type="text" value="{{$course->id}}">
                        <button type="submit" class="p-2 w-full rounded-md bg-blue-400 text-white text-center font-bold">Tambahkan ke paket</button>
                    </form>
                </div>
            </div>
        @empty
