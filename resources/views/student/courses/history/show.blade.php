<x-app-layout>
    ...
    <div class="sm:grid grid-cols-4 gap-4 flex flex-col-reverse"  x-data="{questions:{{ Js::from($course)}}, choosen_question: {{Js::from($course[0])}} }">
        <div class="col-span-3 bg-white p-4 rounded-md shadow-sm">
            ...
            <h1 class="my-4" x-text="choosen_question.question" @click="console.log(choosen_question)"></h1>
            <template x-for="(answer, index) in choosen_question.answers">
                    <label :for="answer.id"     
                        :class="answer.is_correct == 5 ? 'bg-green-500 font-bold text-white hover:bg-green-400' : (choosen_question.student_answer == answer.id && 'bg-red-500 font-bold text-white hover:bg-red-400' )" class="cursor-pointer hover:bg-blue-50 rounded-md p-4 w-full block border-b-2  has-[:checked]:bg-green-400  has-[:checked]:text-white  has-[:checked]:font-bold">
